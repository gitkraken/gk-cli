...
### Synopsis
  ...
  Use --unassigned to list only issues with no assignee, or --assigned-to <user> to list issues
  assigned to a specific person. --all-assignees, --unassigned and --assigned-to are mutually exclusive.
  ...
  Examples:
    ...
    gk issue list github --connection <tokenId> --org gitkraken --closed
    gk issue list jira --unassigned --project GKDEV
    gk issue list jira --assigned-to john@acme.com --project GKDEV
...
```
      ...
      --assigned-to string   List issues assigned to this user (username/email/accountId, provider-dependent; not supported by Trello/Linear)
      --closed               Return closed/done issues instead of open issues
  ...
  -s, --sync                 Bypass the local cache and fetch issues from the provider (no effect with --all or filter flags, which never use the cache)
      --unassigned           List only unassigned issues (not supported by Trello)
```
